...
def process_eight_digit_numbers(df, cells):
    ...
    for index, col, value in cells:
        ...
        global estimate
        if pd.notnull((df.loc[count, "Mobile_Number"])):
            if count == 1:
                estimate = 0
            continue
        ...
        time.sleep(0.2)
        pyautogui.click(x=940, y=332)  # Replace with actual coordinates
        time.sleep(0.2)
        pyautogui.click(x=940, y=332)  # Replace with actual coordinates
        ...
        pyautogui.click(x=1119, y=331)  # Replace with actual coordinates

        # 5) Wait for 2 seconds
        time.sleep(2)
        pyautogui.click(x=1180, y=628)
        ...
        if count % 10 == 0:
            df.to_excel("updated_sheet.xlsx", index="False")
    return df
